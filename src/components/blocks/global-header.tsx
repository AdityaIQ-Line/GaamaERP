...
import type { UserRole } from "@/lib/gaama-types"
...
export function GlobalHeader({
  ...
  const data = useData()
  const roles: UserRole[] = ["admin", "sales", "warehouse", "operations", "finance", "compliance"]
        ...
        <div className="flex items-center gap-1">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuLabel className="text-xs text-muted-foreground">View as (RBAC)</DropdownMenuLabel>
              {roles.map((role) => (
                <DropdownMenuItem
                  key={role}
                  onClick={() => data.setCurrentRole(role)}
                  className={data.currentRole === role ? "bg-accent" : ""}
                >
                  <Shield strokeWidth={ICON_STROKE_WIDTH} className="size-4" />
